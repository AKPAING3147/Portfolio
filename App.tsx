  ...
  return (
    <>
      ...
      >
        ...
        <ProjectShowcase 
          year="2025"
          title="Next Chapter: AIT"
          description="Currently pursuing advanced studies at the Asian Institute of Technology (AIT) in Thailand. Expanding knowledge in AI, global tech trends, and research."
          align="left"
          ...
          icon={Globe}
          tags={['AIT Thailand', 'Masters', 'Research']}
        ...
        <section className={`min-h-screen flex items-center justify-center relative z-10 px-4 py-20 ${isLoveTheme ? 'bg-white/10' : 'bg-black/20'}`}>
          <div className="max-w-6xl w-full">
              <div className="text-center mb-12">
                <h2 className={`text-3xl md:text-5xl font-retro drop-shadow-[0_0_10px_rgba(34,211,238,0.8)] mb-4 uppercase 
                    ${isAkatsuki ? 'text-red-600 drop-shadow-[0_0_15px_red]' 
                    : isStarWars ? 'text-yellow-400 drop-shadow-[0_0_15px_yellow]' 
                    : isRickAndMorty ? 'text-green-400 drop-shadow-[0_0_15px_lime]'
                    : isLoveTheme ? 'text-pink-600 drop-shadow-[0_0_15px_pink]'
                    : 'text-cyan-400'}`}>
                    Project Archive
                </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {/* Project 1: Aung & Htal Wedding */}
                  <motion.div 
                    initial={{ y: 50, opacity: 0 }}
                    whileInView={{ y: 0, opacity: 1 }}
                    viewport={{ once: true }}
                    className={`bg-gray-900 border-2 rounded-xl overflow-hidden transition-all group flex flex-col 
                        ${isAkatsuki ? 'border-red-600 shadow-[0_0_30px_rgba(220,38,38,0.3)]' 
                        : isStarWars ? 'border-yellow-500 shadow-[0_0_30px_rgba(250,204,21,0.3)]' 
                        : isRickAndMorty ? 'border-green-500 shadow-[0_0_30px_rgba(74,222,128,0.3)]'
                        : isLoveTheme ? 'border-pink-300 shadow-[0_0_30px_rgba(249,168,212,0.5)] bg-white/20'
                        : 'border-pink-500 shadow-[0_0_30px_rgba(236,72,153,0.3)] hover:shadow-[0_0_50px_rgba(236,72,153,0.5)]'}`}
                  >
                        <div className="h-48 md:h-64 relative overflow-hidden">
                          <img 
                            src="https://images.unsplash.com/photo-1515934751635-c81c6bc9a2d8?q=80&w=800&auto=format&fit=crop" 
                            alt="Aung & Htal Wedding" 
                            className={`w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 
                                ${isAkatsuki ? 'grayscale contrast-125' : isStarWars ? 'sepia-[.5] brightness-75' : isRickAndMorty ? 'hue-rotate-[-30deg] contrast-125' : isLoveTheme ? 'contrast-110' : ''}`}
                          />
                          <div className={`absolute inset-0 transition-colors 
                              ${isAkatsuki ? 'bg-red-900/40' 
                              : isStarWars ? 'bg-yellow-900/20' 
                              : isRickAndMorty ? 'bg-green-900/20'
                              : isLoveTheme ? 'bg-pink-500/10'
                              : 'bg-pink-900/20 group-hover:bg-transparent'}`}></div>
                        </div>
                        <div className={`p-8 flex flex-col flex-grow backdrop-blur ${isLoveTheme ? 'bg-white/40 text-black' : 'bg-gray-900/90 text-white'}`}>
                          <div className="flex items-center gap-2 mb-2">
                              <Layout size={18} className={isAkatsuki ? 'text-red-500' : isStarWars ? 'text-yellow-400' : isRickAndMorty ? 'text-green-400' : isLoveTheme ? 'text-pink-600' : 'text-pink-500'} />
                              <span className={`text-xs font-retro uppercase ${isAkatsuki ? 'text-red-400' : isStarWars ? 'text-yellow-300' : isRickAndMorty ? 'text-green-400' : isLoveTheme ? 'text-pink-700' : 'text-pink-400'}`}>Web Application</span>
                          </div>
                          <h3 className={`text-2xl font-hand mb-4 ${isLoveTheme ? 'text-pink-900' : 'text-white'}`}>Aung & Htal Wedding</h3>
                          <p className={`mb-6 font-mono text-sm leading-relaxed flex-grow ${isLoveTheme ? 'text-gray-800' : 'text-gray-400'}`}>
                              A specialized wedding platform built with modern web technologies. Focuses on elegant design and user experience for memorable events.
                          </p>
                          <a 
                            href="https://aungandhtalwedding.netlify.app/" 
                            target="_blank" 
                            rel="noreferrer"
                            className={`inline-flex items-center gap-2 px-6 py-3 font-bold rounded transition-colors w-full justify-center md:w-max 
                                ${isAkatsuki ? 'bg-red-700 hover:bg-red-600 text-white' 
                                : isStarWars ? 'bg-yellow-600 hover:bg-yellow-500 text-black' 
                                : isRickAndMorty ? 'bg-green-600 hover:bg-green-500 text-black'
                                : isLoveTheme ? 'bg-pink-500 hover:bg-pink-400 text-white'
                  ...
                  <motion.div 
                    ...
                    className={`bg-gray-900 border-2 rounded-xl overflow-hidden transition-all group flex flex-col 
                        ${isAkatsuki ? 'border-red-600 shadow-[0_0_30px_rgba(220,38,38,0.3)]' 
                        : isStarWars ? 'border-yellow-500 shadow-[0_0_30px_rgba(250,204,21,0.3)]' 
                        : isRickAndMorty ? 'border-cyan-500 shadow-[0_0_30px_rgba(34,211,238,0.3)]'
                        : isLoveTheme ? 'border-sky-300 shadow-[0_0_30px_rgba(186,230,253,0.5)] bg-white/20'
                        : 'border-green-500 shadow-[0_0_30px_rgba(34,197,94,0.3)] hover:shadow-[0_0_50px_rgba(34,197,94,0.5)]'}`}
                  >
                        <div className="h-48 md:h-64 relative overflow-hidden">
                          <img 
                          ...
                          <div className={`absolute inset-0 transition-colors 
                              ...
                              : isStarWars ? 'bg-yellow-900/20' 
                              : isRickAndMorty ? 'bg-green-900/20'
                              : isLoveTheme ? 'bg-sky-500/10'
                              : 'bg-green-900/20 group-hover:bg-transparent'}`}></div>
                        </div>
                        <div className={`p-8 flex flex-col flex-grow backdrop-blur ${isLoveTheme ? 'bg-white/40 text-black' : 'bg-gray-900/90 text-white'}`}>
                          <div className="flex items-center gap-2 mb-2">
                              <School size={18} className={isAkatsuki ? 'text-red-500' : isStarWars ? 'text-yellow-400' : isRickAndMorty ? 'text-cyan-400' : isLoveTheme ? 'text-sky-600' : 'text-green-500'} />
                              <span className={`text-xs font-retro uppercase ${isAkatsuki ? 'text-red-400' : isStarWars ? 'text-yellow-300' : isRickAndMorty ? 'text-cyan-400' : isLoveTheme ? 'text-sky-700' : 'text-green-400'}`}>Educational Platform</span>
                          </div>
                          <h3 className={`text-2xl font-hand mb-4 ${isLoveTheme ? 'text-sky-900' : 'text-white'}`}>Inet College</h3>
                          <p className={`mb-6 font-mono text-sm leading-relaxed flex-grow ${isLoveTheme ? 'text-gray-800' : 'text-gray-400'}`}>
                              The official digital platform for Inet College. Facilitating computer science education and student resources in Myanmar.
                          </p>
                          <a 
                            ...
                            className={`inline-flex items-center gap-2 px-6 py-3 font-bold rounded transition-colors w-full justify-center md:w-max 
                                ${isAkatsuki ? 'bg-red-700 hover:bg-red-600 text-white' 
                                : isStarWars ? 'bg-yellow-600 hover:bg-yellow-500 text-black' 
                                : isRickAndMorty ? 'bg-cyan-600 hover:bg-cyan-500 text-black'
                                : isLoveTheme ? 'bg-sky-500 hover:bg-sky-400 text-white'
